# Boards

## Project Views

### Projects List
- main projects table

### Project Details
- Project Manager (PM) main page
- summary + goal + context + epics + tasks + staffing + budget

## Delivery Boards

### Epic Board
- Kanban board by epic status: `Open | In Progress | Ready for QA | QA | Done`

### Task Board
- Kanban board by task status: `New | InProgress | Self QA | Done`

### My Tasks
- filter by `Assignee = current user`

## QA Boards (Quality Assurance Boards)

### QA Queue (Quality Assurance Queue)
- task status = `Self QA`
- epic status = `Ready for QA` or `QA`

### QA Team Board (Quality Assurance Team Board)
- epic-level Kanban: `Ready for QA | QA | Done`

## Current UI Examples

### Epic board view

This screenshot shows the current epic workspace with summary widgets, an epic table, a priority split chart, and the epic board itself.

![Epic board](../../assets/images/epics-board.png)

### Task table view

This screenshot shows the current task table with delivery, Quality Assurance (QA), hours, dates, and status columns.

![Task table](../../assets/images/tasks-table.png)

## Related Diagram

Page structure: [`../../assets/diagrams/pages-structure.svg`](../../assets/diagrams/pages-structure.svg)
